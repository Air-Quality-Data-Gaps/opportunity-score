...
# load data
# aqli data
aqli <- read.csv(glue("{dir}/data/input/aqli_gadm0_data.csv")) %>% 
...
open_data <- read.csv(glue("{dir}/data/input/AQ Monitoring with AQLI Names.csv"))
# air quality standards data
naaqs <- read.csv(glue("{dir}/data/input/country_annual_average_pm2.5_standards_asInJan2023 - country_annual_average_pm2.5_standards_asInJan2023.csv")) %>%
...
aq_monitors <- read.csv(glue("{dir}/data/input/no_of_monitors_aqli_names.csv")) %>% 
...
aq_data <- aq_data %>%
  mutate(aq_monitoring = ifelse(Evidence.of.current.government.operated.sponsored.AQ.monitoring == "Yes", "Y", "N"),
         ...
         monitor_density = number_of_locations/population)

# assign score- greater opportunity get a score of 1
opportunity_score <- aq_data %>% arrange(desc(pm2021)) %>%
  select(iso_alpha3, country, population, whostandard, natstandard, pm2021, aq_monitoring, open_data, aq_standard, number_of_locations, monitor_density) %>%
  replace_na(list(aq_monitoring = "N", open_data = "N", number_of_locations = 0, monitor_density = 0)) %>% # NAs coded as N or 0 depending the case
  mutate(aq_mon_dummy = ifelse(aq_monitoring == "N", 1, 0), 
         ...
         num_loc_dummy = ifelse(number_of_locations > median(number_of_locations, na.rm = TRUE), 0, 1),
         mon_dens_dummy = ifelse(monitor_density > median(monitor_density, na.rm = TRUE), 0, 1)) %>%
  rowwise() %>%
  mutate(opportunity_score = sum(aq_mon_dummy, open_data_dummy, aq_std_dummy, pm_dummy, pop_dummy, num_loc_dummy, mon_dens_dummy)) %>%
  arrange(desc(opportunity_score)) %>%
  write_csv(glue("{dir}/data/output/opportunity_score_v4.csv"))
